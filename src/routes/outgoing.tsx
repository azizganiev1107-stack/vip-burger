...
function Outgoing() {
  const [items, setItems] = useState<OutgoingItem[]>([
    { id: INITIAL_ITEMS[0].id, quantity: 0, reason: 'İslatıldı' }
  ])

  const addItem = () => {
    setItems([...items, { id: INITIAL_ITEMS[0].id, quantity: 0, reason: 'İslatıldı' }])
  }
  ...
  const updateItem = (index: number, field: keyof OutgoingItem, value: any) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <div>
        <h1 className="text-3xl font-display font-extrabold text-slate-800">Shıǵıs jazıu</h1>
        <p className="text-slate-500 mt-1">İslatılǵan yamasa basqa maqsette ketken zatlardı dizimge alıu</p>
      </div>
      ...
      <div className="glass-card rounded-3xl p-8 space-y-6">
        <div className="space-y-4">
          {items.map((item, index) => (
            <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end p-4 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <div className="md:col-span-3 space-y-1.5">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Sebep / Eskertpe</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="Mısalı: Kuxnya ushın"
                    value={item.reason}
        ...
        <div className="flex flex-col md:flex-row gap-4 pt-4 border-t border-slate-100">
          ...
          <button className="btn-primary flex-[2] flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-700 hover:shadow-orange-500/30">
            <Save className="w-5 h-5" /> Shıǵıstı saqlau
          </button>
        </div>
      </div>
    </div>
